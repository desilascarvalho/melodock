...
function scCard(artist) {
  const action = artist.in_library
    ? '<a href="artist.html?id=' + artist.library_id + '" class="block w-full py-2 rounded-el bg-white/[0.04] text-accent-light text-xs font-medium text-center hover:bg-white/[0.08] transition-colors">✓ Ver artista →</a>'
  ...
  return (
    '<div class="bg-surface rounded-card border border-white/5 p-5 flex flex-col items-center text-center" data-card="' + artist.deezer_id + '">' +
      MD.img(artist.picture_url, artist.name, 'w-20 h-20 rounded-full text-lg', 'rounded-full') +
      '<div class="mt-3 font-semibold text-sm leading-tight">' + MD.esc(artist.name) + '</div>' +
      '<div class="text-xs text-txt-sec mt-1">' + (artist.nb_album ?? artist.albums ?? '?') + ' álbuns · ' + MD.fmtFans(artist.followers ?? artist.fans ?? 0) + ' fãs</div>' +
...
function scOpenModal(artist) {
  ...
  document.getElementById('modal-photo').innerHTML = MD.img(artist.picture_url, artist.name, 'w-14 h-14 text-base', 'rounded-full');
  document.getElementById('modal-name').textContent = artist.name;
  document.getElementById('modal-meta').textContent = (artist.nb_album ?? artist.albums ?? '?') + ' álbuns · ' + MD.fmtFans(artist.followers ?? artist.fans ?? 0) + ' fãs no Deezer';
