...
def dashboard():
    ...
    elif sort == 'name_desc': order = "ORDER BY name DESC"
    elif sort == 'date_asc': order = "ORDER BY added_at ASC"
    else: order = "ORDER BY added_at DESC"
    ...
    for art in artists_raw:
        ...
        if pending: a_dict['visual_status'] = 'syncing'
        elif error: a_dict['visual_status'] = 'error'
        else: a_dict['visual_status'] = 'ok'
        
        artists_list.append(a_dict)
    
    return render_template('dashboard.html', stats=stats, artists_list=artists_list, current_sort=sort, current_page=page, total_pages=total_pages)
...
def downloads():
    ...
    for i in raw:
        ...
        if i['status'] == 'high_priority': prio[i['artist']] = True
...
def settings():
    ...
    if request.method == 'POST':
        if request.form.get('deezer_arl'): db.set_setting('deezer_arl', request.form.get('deezer_arl').strip())
        if request.form.get('download_quality'): db.set_setting('download_quality', request.form.get('download_quality'))
        if request.form.get('ignored_keywords'): db.set_setting('ignored_keywords', request.form.get('ignored_keywords'))
        if request.form.get('max_tracks'): db.set_setting('max_tracks', request.form.get('max_tracks'))
        if request.form.get('scan_time'): db.set_setting('scan_time', request.form.get('scan_time'))
                ...
                for alb in albums:
                    ...
                    if match:
                        if not db.query("SELECT 1 FROM queue WHERE deezer_id=?", (alb['deezer_id'],), one=True):
                            ...
                            tracks = meta.get_album_tracks(alb['deezer_id'], fallback_artist=art_data['name'])
                            for t in tracks:
                                db.execute("INSERT INTO tracks (queue_id, deezer_id, title, artist, track_number, status) VALUES (?, ?, ?, ?, ?, 'completed')",
                                           (qid, t['deezer_id'], t['title'], t['artist'], t['track_num']))
                            count += 1
