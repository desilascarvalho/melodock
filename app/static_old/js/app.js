...
/* Artist picture: real photo from API with gradient fallback */
MD.artistImg = (artistId, name, cls) => {
  const fallback = MD.cover(name || '?', cls);
  if (!artistId) return fallback;
  const src = '/api/artists/' + artistId + '/picture';
  return (
    '<div class="' + (cls || '') + ' relative overflow-hidden shrink-0">' +
      '<img src="' + src + '" alt="' + MD.esc(name || '') + '" ' +
...
MD.badge = (status) => {
  const map = {
    ...
    uploading:    'bg-blue-500/15 text-blue-400 animate-pulse',
    verifying:    'bg-yellow-500/15 text-yellow-400 animate-pulse',
    error:        'bg-red-500/10 text-red-400',
    upload_error: 'bg-red-500/10 text-red-400',
    queued:       'bg-white/[0.06] text-txt-sec',
    skipped:      'bg-white/[0.06] text-txt-muted',
  };
  const label = { upload_error: 'up_error', verifying: 'verif…' }[status] || status;
  return '<span class="font-mono text-[11px] px-2 py-0.5 rounded-md ' + (map[status] || map.queued) + '">' + MD.esc(label) + '</span>';
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // WebSocket — atualiza sidebar em tempo real
  wsConnect();
  wsOn('job_progress', (data) => {
    if (data && data.status) updateSidebarStatus(data);
  });
  wsOn('job_done', () => pollGlobal());
  wsOn('queue_updated', (data) => {
    if (data && data.pending_count != null) updateQueueBadge(data.pending_count);
  });

  pollGlobal();
  setInterval(pollGlobal, 5000);
});
